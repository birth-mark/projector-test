...
const buyBookForUser = (bookId, userId, callback) => {
  const errorCheck = (func, err, ...other) => {
    if (err) return callback(err)
    return func.apply(this, other);
  }

  const bookCheck = (userBooks) => {
    if (userBooks.includes(bookId)) {
      callback(`User already has book with id=${bookId}`);
    } else {
      database.buyBook(bookId, (err) => errorCheck(callback, err, null, 'Success'))
    }
  }

  database.getUser(userId, (err, user ) =>
    errorCheck(database.getUsersBook, err, userId, (err, userBooks) =>
      errorCheck(bookCheck, err, userBooks)
    )
  )
};
